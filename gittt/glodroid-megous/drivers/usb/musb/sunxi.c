...
#define  USBC_REG_test_bit_b(bp, reg)   (readb(reg) & (1 << (bp)))
#define  USBC_REG_test_bit_w(bp, reg)   (readw(reg) & (1 << (bp)))
#define  USBC_REG_test_bit_l(bp, reg)   (readl(reg) & (1 << (bp)))
...
static void USBC_EnableIdPullUp(__iomem void *base)
{
        ...
        reg_val = readb(base+USBC_REG_o_ISCR);
        reg_val |= BIT(USBC_BP_ISCR_ID_PULLUP_EN);
        reg_val = USBC_WakeUp_ClearChangeDetect(reg_val);
        writeb(reg_val, base + USBC_REG_o_ISCR);
}
...
static irqreturn_t sunxi_musb_interrupt(int irq, void *__hci)
{
	...
	unsigned long flags;
        ...
        //dev_warn(musb->controller,"MUSB DBG IRQ \r");
...
static int sunxi_musb_set_mode(struct musb *musb, u8 mode)
{
	...
	u32 mmode = 0;
	dev_warn(musb->controller,"MUSB DBG set_mode \r");
	...
	mmode = USBC_REG_test_bit_l(USBC_BP_ISCR_MERGED_ID_STATUS,
                                musb->mregs + USBC_REG_o_ISCR);

	dev_warn(musb->controller,"MUSB DBG get_id_from_iscr mode = %d\r", mmode);

	mmode = USBC_REG_test_bit_b(USBC_BP_DEVCTL_B_DEVICE,
                                musb->mregs + SUNXI_MUSB_DEVCTL);

        dev_warn(musb->controller,"MUSB DBG get_id_from_b_device mode = %d\r", mmode);
