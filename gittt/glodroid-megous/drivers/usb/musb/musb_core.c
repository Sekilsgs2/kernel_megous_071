				...
				u8 devctl)
{
	...
	//dev_warn(musb->controller,"MUSB DBG musb_stage0_irq\r");
...
static int musb_core_init(u16 musb_type, struct musb *musb)
{
	...
	/* log release info */
	musb->hwvers = 0;//musb_readw(mbase, MUSB_HWVERS);
...
static void musb_save_context(struct musb *musb)
{
	...
	musb->context.busctl = 0;//musb_readb(musb_base, MUSB_ULPI_BUSCONTROL);
